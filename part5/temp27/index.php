...
if(isset($_POST['newdir'])){
	...
	if($newdir == ''){
		...
		if(fetchRow($sql)){
			...
			//查询父ID文件夹的路径
			$sql = "select folder_pid from netdisk_folder where folder_id = $folder_id";
			$parent_path = fetchRow($sql);
			$parent_path = 	$parent_path['folder_path'];
			$parent_path = $parent_path ? "$parent_path,$folder_id" : $folder_id;
			//将新文件夹保存到数据库中
			$sql = "insert into netdisk_folder
			(folder_name,folder_time,folder_path,folder_pid)
			values ('$newdir',now(),'$parent_path',$folder_id)";
			if(!mysql_query($sql)){
	...
	if($upload_file['error'] >0){
		...
		if($file_path){
			...
			if(move_uploaded_file($upload_file['tmp_name'],$file_save_path)){
				...
				//添加到数据库记录
				$sql = "insert into netdisk_file 
				(file_name,file_save,file_size,file_time,folder_id)
				 values('$filename', '$file_save_path','$file_size',now(),$folder_id)";
				if(!mysql_query($sql)){
